    ...
    try:
        ...
        for row in data:
            ...
            if ar_value is not None:
                result_dict[ar_value] = result_dict.get(ar_value, 0) + (
                    az_value if isinstance(az_value, (int, float)) else 0)
        ...
        # 写入表头
        new_sheet.append(["SKU", "Outbound Qty/出库数量"])

        # 写入汇总结果
        for ar, total in result_dict.items():
            new_sheet.append([ar, total])
