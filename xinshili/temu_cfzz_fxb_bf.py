...
def write_data_to_sheet(sheet1, sheet2, columns1, columns2, exchange_rate):
    ...
    try:
        ...
        for row in range(3, sheet1.max_row + 1):  # 跳过表1的第2行
            ...
            # 目前补发运费免除
            openpyxl_utils.set_cell_value(sheet2, target_row, columns2["运费"], 0)
...
def calculate_rmb_prices(file_path):
    ...
    try:
        ...
        total_rows = sheet.range(f"{'B'}{sheet.cells.last_cell.row}").end('up').row
...
def main():
    try:
        ...
        # 查找列
        columns1 = openpyxl_utils.find_columns(sheet1, ["Order ID", "Quantity", "SKU ID"])
        ...
        new_file_path = new_file_dir + "CZFF供应商对账表-补发-" + str(lists[0]) + "元-" \
